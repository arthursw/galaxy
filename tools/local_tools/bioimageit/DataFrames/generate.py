    ...
    def processDataFrame(self, dataFrame, argsList):
        ...
        for args in argsList:
            if args.numbers is not None and len(args.numbers)>0:
                values = self.parseNumbers(args.numbers)
            elif args.strings is not None and len(args.strings)>0:
                values = args.strings.replace('[', '').replace(']', '').split(',')
