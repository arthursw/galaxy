...
def _setup_execution_environment(galaxy_root, use_wetlands):
    """
    Initialize execution environment, working directory, and Wetlands if needed.

    Returns:
        tuple: (environment_manager, working_dir, params_file)
    """
    ...
    return environment_manager, working_dir, params_file


def _load_workflow_config(workflow_file, galaxy_root, tool_conf_xml, inputs_json):
    """
    Load workflow configuration, user inputs, and tool mapping.

    Returns:
        tuple: (user_inputs, tool_map, workflow)
    """
    ...
    try:
        ...
        return user_inputs, tool_map, None

    return user_inputs, tool_map, workflow


def _generate_output_paths(tool, step_folder, num_iterations, element_id, iter_params):
    """
    Generate output file paths for a tool iteration.

    Returns:
        dict: {output_name: output_file_path}
    """
    iter_output_files = {}
    ...
    for output_name, output_info in tool.outputs.items():
        ...
        if format_source and format_source in iter_params:
            ...
            for _, param_value in iter_params.items():
    ...
    return iter_output_files


def _execute_iteration(
    step_id, tool, iter_idx, slice_dict, base_params, input_connections,
    step_outputs, step_folder, num_iterations, params_file, environment_manager,
    working_dir, dry_run
):
    """
    Execute a single iteration of a tool step.

    Returns:
        tuple: (iter_output_files, output_collections_for_this_iteration)
    """
    ...
    iter_output_files = _generate_output_paths(tool, step_folder, num_iterations, element_id, iter_params)

    # Save original directory and change to step folder
    original_dir = os.getcwd()
    os.chdir(step_folder)

    output_collections_for_iteration = []

    try:
        # Collect for building output collections
        if num_iterations > 1:
            for output_name in iter_output_files:
                output_collections_for_iteration.append((output_name, element_id, iter_output_files[output_name]))
        ...
        if check_params_and_outputs(params_file, step_id, iter_idx, iter_params, iter_output_files):
            ...
            return iter_output_files, output_collections_for_iteration
        ...
        if not dry_run:
            ...
            if environment_manager is not None:
                ...
                exit_code, _ = execute_with_wetlands(
                    environment_manager,
                    tool,
                    command,
                    working_dir
...
def _execute_tool_step(
    step_id, step, tool_cache, step_outputs, working_dir, params_file,
    tool_map, galaxy_root, environment_manager, dry_run
):
    """
    Execute a single tool step with all its iterations.

    Updates step_outputs dictionary with results.
    """
    ...
    if not tool_id:
        if step_type == 'data_collection_input' or step_type == 'data_input':
            # Already handled in the workflow executor main function
            pass
        return
    ...
    for iter_idx, slice_dict in enumerate(iteration_slices):
        try:
            iter_output_files, output_collections_for_iteration = _execute_iteration(
                step_id, tool, iter_idx, slice_dict, base_params, input_connections,
                step_outputs, step_folder, num_iterations, params_file, environment_manager,
                working_dir, dry_run
            )

            # Collect elements for building output collections
            for output_name, element_id, output_path in output_collections_for_iteration:
                output_collections[output_name].append(
                    CollectionElement(element_id, output_path)
                )
...
def execute_workflow(workflow_file, tool_conf_xml=None, galaxy_root=None, inputs_json=None, use_wetlands=True, dry_run=False):
    ...
    try:
        ...
        environment_manager, working_dir, params_file = _setup_execution_environment(galaxy_root, use_wetlands)

        # Load workflow configuration
        user_inputs, tool_map, workflow = _load_workflow_config(workflow_file, working_dir.parent if galaxy_root is None else galaxy_root, tool_conf_xml, inputs_json)
        ...
        for step_id in sorted(steps.keys(), key=lambda x: int(x) if x.isdigit() else 0):
            ...
            _execute_tool_step(
                step_id, step, tool_cache, step_outputs, working_dir, params_file,
                tool_map, working_dir.parent if galaxy_root is None else galaxy_root,
                environment_manager, dry_run
            )
