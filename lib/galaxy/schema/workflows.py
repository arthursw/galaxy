...
class ExecuteWithWetlandsPayload(Model):
    inputs_json: Optional[Dict[str, Any]] = Field(
        None,
        title="Inputs JSON",
        description="JSON object containing workflow inputs for execution with Wetlands.",
    )
    tool_conf_xml: Optional[str] = Field(
        None,
        title="Tool Configuration XML Path",
        description="Path to the tool_conf.xml file (optional, will use default if not specified).",
    )
    dry_run: bool = Field(
        False,
        title="Dry Run",
        description="If true, only print execution details but do not actually execute the workflow.",
    )


class ExecuteWithWetlandsResponse(Model):
    workflow_file: str = Field(
        ...,
        title="Workflow File",
        description="Path to the generated .ga workflow file.",
    )
    execution_status: str = Field(
        ...,
        title="Execution Status",
        description="Status of the workflow execution (e.g., 'success', 'error', 'dry_run').",
    )
    message: str = Field(
        ...,
        title="Message",
        description="Detailed message about the execution result.",
    )
    outputs: Optional[Dict[str, Any]] = Field(
        None,
        title="Outputs",
        description="Output files generated by the workflow execution (if available).",
    )
